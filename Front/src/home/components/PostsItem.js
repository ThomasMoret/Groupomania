...
import { modifyPost, likePost, deletePicture } from "../../utils/Api";
...
const PostsItem = ({
  ...
  const userIsOwner = () => {
    ...
    if (user._id === userId._id || user.isAdmin) {
      return true;
    }
  ...
  return (
    <li className="posts-item">
      ...
      <Comment postId={postId} fetchPosts={fetchPosts} comments={comments} />
    </li>
  );
};
